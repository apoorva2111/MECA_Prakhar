...
open class VersaPlayerControls: View {
    ...
    ///VersaplayerControlsmuteoption
    @IBOutlet public weak var mutebtn:VersaStatefulButton? = nil
    ...
    open func prepare() {
        ...
        #if os(macOS)
        mutebtn?.target = self
        mutebtn?.action = #selector(mutebtnaction(sender:))
    ...
    @IBAction open func mutebtnaction(sender: Any? = nil) {
        
        if handler.ismuteEnabled {
            mutebtn?.set(active: false)
            MPVolumeView.setVolume(0.0)
        }else{
            mutebtn?.set(active: true)
