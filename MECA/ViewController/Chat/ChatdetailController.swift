...
class ChatdetailController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let databasemainroot = Constants.refs.databaseChats

        print("ref....\(ref)")
        
        
        let userID = Auth.auth().currentUser?.uid
        ref.child("admin-user").child("1").observeSingleEvent(of: .value, with: { (snapshot) in
          // Get user value
            
            let datasds = try? JSONSerialization.data(withJSONObject: snapshot.value)
            ...
            for snap in snapshot.children {
                ...
                print("stadiumSnap... \(stadiumSnap.childrenCount)")
