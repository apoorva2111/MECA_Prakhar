...
class WhatsNewVM: BaseTableViewVM {
    ...
    override func didSelectRowAt(_ indexPath: IndexPath, tableView: UITableView) {
        ...
        (actualController as! WhatsNewVC).navigationController?.pushViewController(vc, animated: true)
    }
        ...
        APIClient.wevserviceForHomeFeed { (result) in
            ...
            if let respCode = result.resp_code{
                ...
                if respCode == 200{
                    ...
                    (self.actualController as! WhatsNewVC).tblView.reloadData()
                }else{
                    GlobalObj.displayLoader(true, show: false)
